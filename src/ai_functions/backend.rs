...
pub fn print_rest_api_endpoints(_code_input: &str) {
    ...
    /// MUST READ: The values in "is_route_dynamic" fields are NOT TO BE WRAPPED in double quotes. They should only be `true` or `false`, as proper JSON boolean values
    /// EXAMPLE:
    /// INPUT_CODE:
    /// ...
    /// pub struct Item {
